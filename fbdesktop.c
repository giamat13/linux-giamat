 ...
 * browser), plus a taskbar. Draws directly to /dev/fb0, reads mouse from
 * /dev/input/mice and keyboard from stdin in raw mode.
...
/* defined below, next to the other window plumbing */
...
static void draw_browser(struct window *w)
{
	int cy = w->y + TITLE_H;
...
static void browser_pointer(int lx, int ly, int left, int right)
{
	if (!xdpy)
		return;
	if (lx < 0) lx = 0;
...
static void browser_keys(const char *buf, int n)
{
	if (!xdpy)
		return;
	for (int i = 0; i < n; i++) {
