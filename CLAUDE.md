...
This list describes where past changes happened to land, **not a restriction on where future ones may land, and not a list of the only directories you're allowed to edit.** There is no allowlist of editable paths in this repo — every file, including files never touched before (kernel core, `arch/`, `drivers/`, `Kconfig`, `Documentation/`, anywhere else upstream) is fair game when the task needs it. Pick whichever files the task actually needs — including kernel subsystems, Kconfig, or drivers — rather than defaulting to the userspace desktop (`tools/fbdesktop/`, `tools/apps/`) out of habit just because that's where most past work happened to land.
...
**Build every window as a real, drawn UI — never a terminal.** The character grid (`gch/gfg/gbg`) exists for things that are genuinely a stream of text: the live terminal and one-shot command output. It is not a shortcut for building an app by dumping `ps`/`df`/`free` text into a grid — that is what the file manager and task manager used to do, and it read as a terminal, not an application. A window kind draws its own pixels from the primitives in `draw.c` (rects, rounded rects, gradients, circles, text, small hand-drawn icons), the way `files.c` (`draw_files`), `taskmgr.c` (`draw_taskmgr`) and `settings.c` do: real toolbars/sidebars, column headers, per-row icons, selection/hover highlights, graphs and usage bars. The pattern: give the type its own `draw_<type>()`, have `draw_window()` (`window.c`) dispatch to it and `return` before the generic grid path, and add a matching `<type>_click()` that hit-tests the exact pixels you drew (keep layout in shared helpers so the renderer and the click handler can't drift apart). Read the underlying data straight from `/proc`, `statvfs`, `readdir`, etc. — not by shelling out and parsing text.
